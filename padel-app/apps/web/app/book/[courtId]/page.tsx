...
import { Loader2, AlertTriangle, ArrowLeft, CheckCircle, Users, ShieldCheck, UserX } from 'lucide-react';
...
function BookingPageInternal() {
  ...
  useEffect(() => {
    if (courtId) {
        // In a real app, you might fetch full court details if not passed or to verify
        setCourtInfo({
            id: courtId,
  ...
  const fetchTimeSlots = useCallback(async () => {
    ...
    try {
      ...
      setErrorSlots(err.message || "An unexpected error occurred fetching slots.");
      setTimeSlots([]);
    } finally {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/bookings/`, {
        ...
        body: JSON.stringify({
          ...
          start_time: selectedTimeSlot.start_time, // Send ISO string
        }),
      });
      const data: BookingDetail = await response.json();
      if (!response.ok) {
        const errorData: ApiError = await response.json().catch(() => ({ detail: "Booking failed. Please try again." }));
        const errorMessage = typeof errorData.detail === 'string' ? errorData.detail : "Booking failed. Please try again.";
        throw new Error(errorMessage);
      }
      toast.success("Booking confirmed! Now set up your game.");
      setCreatedBookingDetails(data); // Store booking details for game creation
      setSelectedTimeSlot(null); // Reset selection
      // router.push('/bookings'); // Redirect to user's bookings page
      fetchTimeSlots(); // Refresh slots for the current date
    } catch (err: any) {
  ...
  const fetchGameDetails = useCallback(async (gameIdToFetch: number) => {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData: ApiError = await response.json().catch(() => ({ detail: "Failed to fetch game details" }));
            const msg = typeof errorData.detail === 'string' ? errorData.detail : "Failed to fetch game details";
            throw new Error(msg);
        }
        const gameData: GameResponse = await response.json();
        setCreatedGame(gameData); // Update the game state with fresh details including new player
    } catch (err: any) {
        console.error("Error fetching game details:", err);
        toast.error(err.message || "Could not refresh game details.");
    }
  }, [accessToken]);
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/v1/games/`, {
            ...
            body: JSON.stringify({
                ...
                game_type: gameType,
                skill_level: skillLevel || null, // Send null if empty
            }),
        });
        const data: GameResponse = await response.json();
        if (!response.ok) {
            const errorData: ApiError = await response.json().catch(() => ({ detail: "Failed to create game." }));
            const errorMessage = typeof errorData.detail === 'string' ? errorData.detail : "Failed to create game.";
            throw new Error(errorMessage);
        }
        toast.success("Game created successfully! Now invite players.");
        setCreatedGame(data);
        setCreatedBookingDetails(null); // Hide game creation form
    } catch (err: any) {
        console.error("Game creation error:", err);
        toast.error(err.message || "Failed to create game.");
        setGameCreationError(err.message || "Failed to create game.");
    } finally {
        setIsCreatingGame(false);
    }
  };
  
  const handlePlayerInvited = (invitedPlayer: any) => { // `any` for now, should be UserSearchResult
    // After a player is successfully invited via UserSearchAndInvite component,
    // refetch the game details to update the player list.
    if (createdGame) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4 space-y-6">
        ...
        {!createdBookingDetails && !createdGame && (
            <>
                ...
                {selectedDate && (
                    <Card>
                        <CardHeader>
                            <CardTitle>Select Time Slot for {format(selectedDate, 'PPP')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            {isLoadingSlots && (
                            ...
                            {errorSlots && (
                                <div className="text-center py-6 text-destructive">
                                    ...
                                    <p>{errorSlots}</p>
                                    <Button variant="outline" onClick={fetchTimeSlots} className="mt-3">Try Again</Button>
                                </div>
                            )}
                            {!isLoadingSlots && !errorSlots && timeSlots.length === 0 && (
                                <p className="text-center text-muted-foreground py-6">No available slots for this date.</p>
                            )}
                            {!isLoadingSlots && !errorSlots && timeSlots.length > 0 && (
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                                    {timeSlots.map((slot) => (
                                        <Button 
                                            ...
                                            className={`w-full ${!slot.is_available ? 'cursor-not-allowed opacity-50' : ''}`}
                                        >
        ...
        {createdBookingDetails && !createdGame && (
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Your booking for {courtInfo.name} at {format(parseISO(createdBookingDetails.start_time), 'PPP, HH:mm')} is confirmed. Now set up your game details.
                    </CardDescription>
                </CardHeader>
                <form onSubmit={handleCreateGame}>
                    <CardContent className="space-y-4">
                        <div>
                            ...
                            <RadioGroup defaultValue="PRIVATE" value={gameType} onValueChange={(value: GameTypeOption) => setGameType(value)} className="mt-2 flex space-x-4">
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="PRIVATE" id="gameTypePrivate" />
                                    <Label htmlFor="gameTypePrivate" className="font-normal flex items-center"><ShieldCheck className="mr-2 h-4 w-4 text-muted-foreground"/> Private</Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="PUBLIC" id="gameTypePublic" />
                                    <Label htmlFor="gameTypePublic" className="font-normal flex items-center"><Users className="mr-2 h-4 w-4 text-muted-foreground"/> Public</Label>
                                </div>
                            </RadioGroup>
                            <p className="text-xs text-muted-foreground mt-1">
                                Private games are only visible to invited players. Public games can be discovered and joined by other users.
                            </p>
                        </div>
                        <div>
                            <Label htmlFor="skillLevel" className="text-base font-medium">Skill Level (Optional)</Label>
                            <Input 
                                id="skillLevel" 
                                value={skillLevel} 
                                onChange={(e) => setSkillLevel(e.target.value)} 
                                placeholder="E.g., Beginner, Intermediate, Advanced 3.5+"
                                className="mt-2"
                            />
                        </div>
                        {gameCreationError && <p className="text-sm text-destructive">{gameCreationError}</p>}
                    </CardContent>
                    <CardFooter>
                        <Button type="submit" className="w-full" disabled={isCreatingGame}>
                            {isCreatingGame ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating Game...</> : "Create Game & Invite Players"}
                        </Button>
                    </CardFooter>
                </form>
            </Card>
        ...
        {createdGame && (
            <Card>
                <CardHeader>
                    <CardTitle>Manage Game: {createdGame.game_type}</CardTitle>
                    <CardDescription>
                        Booking ID: {createdGame.booking_id}. Game ID: {createdGame.id}. 
                        {createdGame.skill_level && `Skill Level: ${createdGame.skill_level}`}
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div>
                        <h4 className="text-md font-semibold mb-2">Current Players ({createdGame.players.length} / 4)</h4>
                        {createdGame.players.length > 0 ? (
                            <ul className="space-y-2">
                                {createdGame.players.map((playerEntry) => (
                                    <li key={playerEntry.user.id} className="flex justify-between items-center p-2 border rounded-md">
                                        <span>{playerEntry.user.name || playerEntry.user.email}</span>
                                        <Badge variant={getPlayerStatusVariant(playerEntry.status)}>{playerEntry.status}</Badge>
                                    </li>
                                ))}
                            </ul>
                        ) : (
                            <p className="text-sm text-muted-foreground">No players yet (except you, the creator).</p>
                        )}
                    </div>
                    {createdGame.players.length < 4 && (
                         <UserSearchAndInvite 
                            gameId={createdGame.id} 
                            currentPlayers={createdGame.players} 
                            onPlayerInvited={handlePlayerInvited} 
                        />
                    )}
                    {createdGame.players.length >= 4 && 
                        <p className="text-sm text-green-600 font-medium">This game is now full!</p>
                    }
                </CardContent>
            </Card>
        )}
