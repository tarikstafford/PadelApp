...
def fix_inconsistencies(db: Session, inconsistencies: List[Dict[str, Any]], dry_run: bool = True) -> int:
    """Fix ELO range inconsistencies."""
    fixed_count = 0
    
    for inconsistency in inconsistencies:
        if inconsistency['type'] == 'tournament_category_config':
            config = db.query(TournamentCategoryConfig).filter(
                TournamentCategoryConfig.id == inconsistency['id']
            ).first()
            ...
            if config:
                if not dry_run:
                    ...
                    print(f"✅ Fixed tournament category config {config.id}: {inconsistency['category']} -> {inconsistency['expected_min_elo']}-{inconsistency['expected_max_elo']}")
                else:
                    print(f"🔍 Would fix tournament category config {config.id}: {inconsistency['category']} -> {inconsistency['expected_min_elo']}-{inconsistency['expected_max_elo']}")
                fixed_count += 1
        
        elif inconsistency['type'] == 'recurring_tournament_template':
            template = db.query(RecurringTournamentCategoryTemplate).filter(
                RecurringTournamentCategoryTemplate.id == inconsistency['id']
            ).first()
            ...
            if template:
                if not dry_run:
                    ...
                    print(f"✅ Fixed recurring tournament template {template.id}: {inconsistency['category']} -> {inconsistency['expected_min_elo']}-{inconsistency['expected_max_elo']}")
                else:
                    print(f"🔍 Would fix recurring tournament template {template.id}: {inconsistency['category']} -> {inconsistency['expected_min_elo']}-{inconsistency['expected_max_elo']}")
                fixed_count += 1
...
def main():
    ...
    dry_run = not should_fix
    
    if should_fix:
        print("🔧 FIX MODE: Will apply fixes to database")
    else:
        print("🔍 CHECK MODE: Will only report inconsistencies (use --fix to apply fixes)")
    print()
    ...
    try:
        ...
        if config_inconsistencies:
            print(f"❌ Found {len(config_inconsistencies)} inconsistencies in tournament category configs:")
            for inconsistency in config_inconsistencies:
                print(f"  Tournament {inconsistency['tournament_id']}, Category {inconsistency['category']}: "
        ...
        if template_inconsistencies:
            print(f"❌ Found {len(template_inconsistencies)} inconsistencies in recurring tournament templates:")
            for inconsistency in template_inconsistencies:
                print(f"  Recurring Tournament {inconsistency['recurring_tournament_id']}, Category {inconsistency['category']}: "
                      f"{inconsistency['current_min_elo']}-{inconsistency['current_max_elo']} "
                      f"(expected: {inconsistency['expected_min_elo']}-{inconsistency['expected_max_elo']})")
        else:
        ...
        if all_inconsistencies:
            if should_fix:
                ...
                fixed_count = fix_inconsistencies(db, all_inconsistencies, dry_run=False)
