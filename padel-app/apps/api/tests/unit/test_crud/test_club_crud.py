...
class TestClubCRUD:
    ...
    def test_create_club_minimal_data(self, db_session: Session, test_owner: User):
        ...
        club_data = ClubCreate(name=fake.company(), owner_id=test_owner.id)
    ...
    def test_get_clubs_basic(self, db_session: Session, test_owner: User):
        ...
        club1_data = ClubCreate(
            name="Club Alpha", city="New York", owner_id=test_owner.id
        )
        club1 = crud.club_crud.create_club(db_session, club1_data)

        club2_data = ClubCreate(
            name="Club Beta", city="Los Angeles", owner_id=test_owner.id
        )
    ...
    def test_get_clubs_with_pagination(self, db_session: Session, test_owner: User):
        ...
        for i in range(5):
            club_data = ClubCreate(
                name=f"Club {i}", city=fake.city(), owner_id=test_owner.id
            )
    ...
    def test_get_clubs_with_name_filter(self, db_session: Session, test_owner: User):
        ...
        club1_data = ClubCreate(
            name="Tennis Club Alpha", city="New York", owner_id=test_owner.id
        )
        crud.club_crud.create_club(db_session, club1_data)

        club2_data = ClubCreate(
            name="Padel Club Beta", city="Los Angeles", owner_id=test_owner.id
        )
        crud.club_crud.create_club(db_session, club2_data)

        club3_data = ClubCreate(
            name="Sports Center Gamma", city="Chicago", owner_id=test_owner.id
        )
        crud.club_crud.create_club(db_session, club3_data)
    ...
    def test_get_clubs_with_city_filter(self, db_session: Session, test_owner: User):
        ...
        club1_data = ClubCreate(name="Club A", city="New York", owner_id=test_owner.id)
        crud.club_crud.create_club(db_session, club1_data)

        club2_data = ClubCreate(name="Club B", city="New York", owner_id=test_owner.id)
        crud.club_crud.create_club(db_session, club2_data)

        club3_data = ClubCreate(
            name="Club C", city="Los Angeles", owner_id=test_owner.id
        )
        crud.club_crud.create_club(db_session, club3_data)
    ...
    def test_get_clubs_case_insensitive_filtering(
        self, db_session: Session, test_owner: User
    ):
        """Test that name and city filtering is case insensitive"""
        club_data = ClubCreate(
            name="Elite Tennis Club", city="New York", owner_id=test_owner.id
        )
    ...
    def test_get_clubs_with_sorting(self, db_session: Session, test_owner: User):
        ...
        # Create clubs with different names
        club1_data = ClubCreate(
            name="Zebra Club", city="New York", owner_id=test_owner.id
        )
        crud.club_crud.create_club(db_session, club1_data)

        club2_data = ClubCreate(
            name="Alpha Club", city="Boston", owner_id=test_owner.id
        )
        crud.club_crud.create_club(db_session, club2_data)

        # Test sorting by name ascending
        clubs_asc = crud.club_crud.get_clubs(
            db_session, sort_by="name", sort_desc=False
        )
        assert clubs_asc[0].name == "Alpha Club"
        assert clubs_asc[1].name == "Zebra Club"

        # Test sorting by name descending
        clubs_desc = crud.club_crud.get_clubs(
            db_session, sort_by="name", sort_desc=True
        )
        assert clubs_desc[0].name == "Zebra Club"
        assert clubs_desc[1].name == "Alpha Club"

        # Test sorting by city
        clubs_city = crud.club_crud.get_clubs(
            db_session, sort_by="city", sort_desc=False
        )
    ...
    def test_get_clubs_invalid_sort_field(self, db_session: Session, test_owner: User):
        ...
        club_data = ClubCreate(name="Test Club", owner_id=test_owner.id)
        club = crud.club_crud.create_club(db_session, club_data)
    ...
    def test_update_club_success(self, db_session: Session, test_owner: User):
        ...
        # Create club
        club_data = ClubCreate(
            name="Original Name", city="Original City", owner_id=test_owner.id
        )
        created_club = crud.club_crud.create_club(db_session, club_data)
    ...
    def test_update_club_with_times(self, db_session: Session, test_owner: User):
        ...
        # Update times
        update_data = ClubUpdate(opening_time=time(6, 30), closing_time=time(23, 30))
    ...
    def test_update_club_empty_string_fields(
        self, db_session: Session, test_owner: User
    ):
        """Test updating club with empty string fields"""
        # Create club
        club_data = ClubCreate(
            name="Test Club", description="Original description", owner_id=test_owner.id
        )
        created_club = crud.club_crud.create_club(db_session, club_data)

        # Update with empty string (should be converted to None)
        update_data = ClubUpdate(description="", amenities="")
    ...
    def test_update_club_all_fields(self, db_session: Session, test_owner: User):
        ...
        # Create club
        club_data = ClubCreate(name="Original Club", owner_id=test_owner.id)
        created_club = crud.club_crud.create_club(db_session, club_data)
    ...
    def test_club_repr(self, db_session: Session, test_owner: User):
        ...
        club_data = ClubCreate(
            name="Test Club", city="Test City", owner_id=test_owner.id
        )
        created_club = crud.club_crud.create_club(db_session, club_data)
    ...
    def test_create_club_with_invalid_owner(self, db_session: Session):
        ...
        club_data = ClubCreate(name="Test Club", owner_id=99999)  # Non-existent user
    ...
    def test_get_clubs_with_filters_no_matches(
        self, db_session: Session, test_owner: User
    ):
    ...
    def test_get_clubs_combined_filters(self, db_session: Session, test_owner: User):
        ...
        club2_data = ClubCreate(
            name="Tennis Club Beta", city="Los Angeles", owner_id=test_owner.id
        )
        crud.club_crud.create_club(db_session, club2_data)

        club3_data = ClubCreate(
            name="Padel Club Alpha", city="New York", owner_id=test_owner.id
        )
        crud.club_crud.create_club(db_session, club3_data)
    ...
    def test_update_club_model_dump_exclude_unset(
        self, db_session: Session, test_owner: User
    ):
