...
class TestAuthenticationMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_with_valid_bearer_token(self, middleware, mock_request, mock_user, mock_token_data):
        ...
        # Mock dependencies
        with patch('app.middleware.auth.decode_token_payload', return_value=mock_token_data):
            with patch('app.middleware.auth.user_crud.get_user_by_email', return_value=mock_user):
                with patch('app.middleware.auth.SessionLocal') as mock_session_local:
                    mock_db = MagicMock()
    ...
    async def test_dispatch_with_invalid_bearer_token(self, middleware, mock_request):
        ...
        with patch('app.middleware.auth.decode_token_payload', side_effect=Exception("Invalid token")):
            with patch('app.middleware.auth.SessionLocal') as mock_session_local:
                mock_db = MagicMock()
                mock_session_local.return_value = mock_db
    ...
    @pytest.mark.asyncio
    async def test_dispatch_with_user_not_found(self, middleware, mock_request, mock_token_data):
        ...
        # Mock dependencies
        with patch('app.middleware.auth.decode_token_payload', return_value=mock_token_data):
            with patch('app.middleware.auth.user_crud.get_user_by_email', return_value=None):
                with patch('app.middleware.auth.SessionLocal') as mock_session_local:
                    mock_db = MagicMock()
                    mock_session_local.return_value = mock_db
    ...
    @pytest.mark.asyncio
    async def test_dispatch_handles_database_exception(self, middleware, mock_request, mock_token_data):
        ...
        # Mock dependencies
        with patch('app.middleware.auth.decode_token_payload', return_value=mock_token_data):
            with patch('app.middleware.auth.user_crud.get_user_by_email', side_effect=Exception("Database error")):
                with patch('app.middleware.auth.SessionLocal') as mock_session_local:
                    mock_db = MagicMock()
                    mock_session_local.return_value = mock_db
    ...
    @pytest.mark.asyncio
    async def test_dispatch_case_insensitive_bearer_scheme(self, middleware, mock_request, mock_user, mock_token_data):
    ...
    @pytest.mark.asyncio
    async def test_dispatch_with_multiple_spaces_in_auth_header(self, middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_dispatch_with_empty_authorization_header(self, middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_dispatch_database_session_cleanup(self, middleware, mock_request, mock_token_data):
...
class TestMiddlewareIntegration:
    ...
    def test_middleware_dependencies_available(self):
        ...
        from app.middleware.auth import decode_token_payload, user_crud, SessionLocal
