...
@pytest.fixture
def test_booking(db_session: Session, test_court: models.Court, player_user: models.User) -> models.Booking:
...
def club_admin_auth_headers(club_admin_user: models.User) -> Dict[str, str]:
    ...
    token = create_access_token(subject=club_admin_user.email, role=club_admin_user.role.value)
    return {"Authorization": f"Bearer {token}"}
...
def super_admin_auth_headers(super_admin_user: models.User) -> Dict[str, str]:
    ...
    token = create_access_token(subject=super_admin_user.email, role=super_admin_user.role.value)
    return {"Authorization": f"Bearer {token}"}
...
def expired_auth_headers() -> Dict[str, str]:
    ...
    token = create_access_token(subject="expired@example.com", role="PLAYER", expires_delta=-1)
    return {"Authorization": f"Bearer {token}"}