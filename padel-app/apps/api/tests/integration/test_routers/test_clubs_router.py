...
class TestClubsRouter:
    ...
    def test_create_club_not_club_admin(
        self, client: TestClient, player_auth_headers: dict, sample_club_data: dict
    ):
    ...
    def test_create_club_admin_already_owns_club(
        self,
        client: TestClient,
        club_admin_auth_headers: dict,
    ...
    def test_create_club_invalid_data(
    ...
    def test_create_club_with_super_admin(
        self, client: TestClient, super_admin_auth_headers: dict, sample_club_data: dict
    ):
    ...
    def test_read_clubs_with_pagination(
        self, client: TestClient, db_session: Session, club_admin_user: models.User
    ):
        ...
        # Test pagination
        response = client.get(
            f"{settings.API_V1_STR}/clubs", params={"skip": 0, "limit": 3}
        )
        assert response.status_code == 200
        data = response.json()
        assert len(data) <= 3

    def test_read_clubs_with_name_filter(
        self, client: TestClient, test_club: models.Club
    ):
        """Test reading clubs with name filter."""
        response = client.get(
            f"{settings.API_V1_STR}/clubs", params={"name": test_club.name}
        )
        assert response.status_code == 200
        data = response.json()
        assert len(data) >= 1
    ...
    def test_read_clubs_with_city_filter(
        self, client: TestClient, test_club: models.Club
    ):
        """Test reading clubs with city filter."""
        response = client.get(
            f"{settings.API_V1_STR}/clubs", params={"city": test_club.city}
        )
        assert response.status_code == 200
        data = response.json()
        assert len(data) >= 1
    ...
    def test_read_clubs_with_descending_sort(
        self, client: TestClient, test_club: models.Club
    ):
    ...
    def test_read_clubs_invalid_pagination(self, client: TestClient):
        ...
        response = client.get(
            f"{settings.API_V1_STR}/clubs", params={"skip": -1, "limit": 0}
        )
        assert response.status_code == 422
    ...
    def test_read_clubs_invalid_filter_values(self, client: TestClient):
        ...
        response = client.get(
            f"{settings.API_V1_STR}/clubs", params={"name": "", "city": ""}
        )
        assert response.status_code == 422
    ...
    def test_read_clubs_filter_too_long(self, client: TestClient):
        ...
        response = client.get(
            f"{settings.API_V1_STR}/clubs", params={"name": long_string}
        )
        assert response.status_code == 422
    ...
    def test_read_club_with_courts(
        self, client: TestClient, test_club: models.Club, test_court: models.Court
    ):
    ...
    def test_read_club_courts_with_pagination(
        self, client: TestClient, test_club: models.Club, db_session: Session
    ):
        """Test reading club courts with pagination."""
        # Create multiple courts
        for i in range(5):
            court_in = schemas.CourtCreate(
                name=f"Court {i}", description=f"Description {i}", club_id=test_club.id
            )
    ...
    def test_read_club_courts_invalid_pagination(
        self, client: TestClient, test_club: models.Club
    ):
    ...
    def test_read_club_courts_limit_exceeded(
        self, client: TestClient, test_club: models.Club
    ):
    ...
    def test_read_club_courts_empty_result(
        self, client: TestClient, db_session: Session, club_admin_user: models.User
    ):
    ...
    def test_club_creation_business_rules(
        ...
        club_data = {"name": "New Admin Club", "description": "Club for new admin"}

        response = client.post(
            f"{settings.API_V1_STR}/clubs", headers=headers, json=club_data
        )
        assert response.status_code == 201

        # Should not be able to create second club
        club_data2 = {"name": "Second Club", "description": "Second club attempt"}

        response2 = client.post(
            f"{settings.API_V1_STR}/clubs", headers=headers, json=club_data2
        )
        assert response2.status_code == 400

    def test_club_data_validation_edge_cases(
        self, client: TestClient, db_session: Session
    ):
        ...
        token = create_access_token(
            subject=validation_admin.email, role=validation_admin.role.value
        )
        headers = {"Authorization": f"Bearer {token}"}
    ...
    def test_club_search_functionality(
        self, client: TestClient, db_session: Session, club_admin_user: models.User
    ):
        ...
        test_clubs = [
            {
                ...
                "city": "Madrid",
                "description": "Club in Madrid",
            },
            {
                "name": "Barcelona Sports",
                "city": "Barcelona",
                "description": "Club in Barcelona",
            },
            {
                "name": "Valencia Tennis",
                "city": "Valencia",
                "description": "Club in Valencia",
            },
        ]
        ...
        # Test name search
        response = client.get(f"{settings.API_V1_STR}/clubs", params={"name": "Madrid"})
        assert response.status_code == 200
        data = response.json()
        ...
        # Test city search
        response = client.get(
            f"{settings.API_V1_STR}/clubs", params={"city": "Barcelona"}
        )
        assert response.status_code == 200
        data = response.json()
        assert any(club["city"] == "Barcelona" for club in data if club["city"])

    def test_club_endpoint_performance(
        self, client: TestClient, test_club: models.Club
    ):
    ...
    def test_club_sql_injection_protection(self, client: TestClient):
        ...
        response = client.get(
            f"{settings.API_V1_STR}/clubs", params={"name": "'; DROP TABLE clubs; --"}
        )
        # Should not cause server error, just return normal response
        assert response.status_code == 200

    def test_club_authorization_boundary_cases(
        self, client: TestClient, sample_club_data: dict
    ):
        ...
        for role, expected_status in roles_and_expected_status:
            if role == "no_token":
                response = client.post(
                    f"{settings.API_V1_STR}/clubs", json=sample_club_data
                )
            else:
    ...
    def test_club_concurrent_creation(self, client: TestClient, db_session: Session):
        ...
        token = create_access_token(
            subject=concurrent_admin.email, role=concurrent_admin.role.value
        )
        headers = {"Authorization": f"Bearer {token}"}
