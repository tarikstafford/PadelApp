...
class TestAuthLogin:
    def test_login_success(self, client: TestClient):
        ...
        with patch("app.crud.user_crud.authenticate_user") as mock_auth, patch(
            ...
            mock_access_token.assert_called_once_with(
                subject="test@example.com", role="PLAYER"
            )
    ...
    def test_login_inactive_user(self, client: TestClient):
        ...
        with patch("app.crud.user_crud.authenticate_user") as mock_auth:
            # Mock inactive user
            inactive_user = User(
                id=1, email="test@example.com", full_name="Test User", is_active=False
            )
...
class TestAuthRegister:
    def test_register_success(self, client: TestClient):
        ...
        with patch("app.crud.user_crud.get_user_by_email", return_value=None), patch(
    ...
    def test_register_missing_required_fields(self, client: TestClient):
        ...
        response = client.post(
            "/api/v1/auth/register",
            json={"password": "password", "full_name": "Test User"},
        )
        assert response.status_code == 422
...
class TestProfilePictureUpload:
    def test_profile_picture_upload_success(
        self, client: TestClient, user_auth_headers: dict
    ):
        """Test successful profile picture upload."""
        with patch(
            "app.services.file_service.upload_profile_picture"
        ) as mock_upload, patch("app.crud.user_crud.update_user") as mock_update:
            ...
            response = client.post(
                "/api/v1/auth/profile-picture", files=files, headers=user_auth_headers
            )
    ...
    def test_profile_picture_upload_unauthorized(self, client: TestClient):
        ...
        response = client.post("/api/v1/auth/profile-picture", files=files)

        assert response.status_code == 401

    def test_profile_picture_upload_invalid_file_type(
        self, client: TestClient, user_auth_headers: dict
    ):
    ...
    def test_profile_picture_upload_no_file(
        self, client: TestClient, user_auth_headers: dict
    ):
        """Test profile picture upload without file."""
        response = client.post(
            "/api/v1/auth/profile-picture", headers=user_auth_headers
        )
...
class TestAuthIntegration:
    def test_register_login_workflow(self, client: TestClient):
        ...
        with patch("app.crud.user_crud.get_user_by_email") as mock_get, patch(
            "app.crud.user_crud.create_user"
        ) as mock_create, patch(
            "app.crud.user_crud.authenticate_user"
        ) as mock_auth, patch(
            "app.core.security.create_access_token"
        ) as mock_access_token, patch(
            "app.core.security.create_refresh_token"
    ...
    def test_login_refresh_workflow(self, client: TestClient):
        ...
        with patch("app.crud.user_crud.authenticate_user") as mock_auth, patch(
            ...
            "app.core.security.decode_token_payload"
...
class TestAuthEdgeCases:
    def test_login_with_special_characters_in_password(self, client: TestClient):
        ...
        with patch("app.crud.user_crud.authenticate_user") as mock_auth:
            ...
            with patch(
                "app.core.security.create_access_token", return_value="token"
