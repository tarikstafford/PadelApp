...
class CourtBookingService:
    ...
    def is_court_available(self, db: Session, court_id: int, start_time: datetime, end_time: datetime) -> bool:
        """
        ...
        tournament_bookings = db.query(TournamentCourtBooking).filter(
        ...
        if tournament_bookings:
            return False
        
        return True
    
    def get_court_availability(self, db: Session, court_id: int, date: datetime) -> List[Dict[str, Any]]:
        """
        ...
        date_start = datetime.combine(date.date(), datetime.min.time().replace(hour=start_hour))
        date_end = datetime.combine(date.date(), datetime.min.time().replace(hour=end_hour))
        ...
        while current_time + slot_duration <= date_end:
            ...
            if self.is_court_available(db, court_id, current_time, slot_end):
                ...
                booking_type = self._get_blocking_booking_type(db, court_id, current_time, slot_end)
                slots.append({
                    'start_time': current_time,
                    'end_time': slot_end,
                    'available': False,
    ...
    def _get_blocking_booking_type(self, db: Session, court_id: int, start_time: datetime, end_time: datetime) -> str:
        """
        ...
        regular_booking = db.query(Booking).filter(
            Booking.court_id == court_id,
            Booking.status.in_([BookingStatus.CONFIRMED, BookingStatus.PENDING]),
            Booking.start_time < end_time,
            Booking.end_time > start_time
        ...
        tournament_booking = db.query(TournamentCourtBooking).filter(
            TournamentCourtBooking.court_id == court_id,
            TournamentCourtBooking.start_time < end_time,
            TournamentCourtBooking.end_time > start_time
        ).first()
    ...
    def get_tournament_blocked_times(self, db: Session, court_id: int, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        """
    ...
    def create_booking_with_validation(self, db: Session, booking_data: Dict[str, Any]) -> Optional[Booking]:
        """
    ...
    def update_tournament_court_usage(self, db: Session, match_id: int, court_booking_id: int):
        """
        ...
        """
        court_booking = db.query(TournamentCourtBooking).filter(
            TournamentCourtBooking.id == court_booking_id
        ).first()
