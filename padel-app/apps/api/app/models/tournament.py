from sqlalchemy import Column, Integer, String, ForeignKey, Enum as SAEnum, DateTime, Boolean, Float, Text
...
class Tournament(Base):
    ...
    categories = relationship("TournamentCategoryConfig", back_populates="tournament", cascade="all, delete-orphan")
    ...
    court_bookings = relationship("TournamentCourtBooking", back_populates="tournament", cascade="all, delete-orphan")
...
class TournamentCategoryConfig(Base):
    ...
    teams = relationship("TournamentTeam", back_populates="category_config", cascade="all, delete-orphan")
...
class TournamentTeam(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    tournament_id = Column(Integer, ForeignKey("tournaments.id"), nullable=False)
    category_config_id = Column(Integer, ForeignKey("tournament_category_configs.id"), nullable=False)
    ...
    matches_as_team1 = relationship("TournamentMatch", foreign_keys="[TournamentMatch.team1_id]", back_populates="team1")
    matches_as_team2 = relationship("TournamentMatch", foreign_keys="[TournamentMatch.team2_id]", back_populates="team2")
...
class TournamentMatch(Base):
    ...
    # Match details
    round_number = Column(Integer, nullable=False)  # 1 for first round, 2 for second, etc.
    ...
    # Bracket progression
    winner_advances_to_match_id = Column(Integer, ForeignKey("tournament_matches.id"), nullable=True)
    loser_advances_to_match_id = Column(Integer, ForeignKey("tournament_matches.id"), nullable=True)  # For double elimination
    ...
    team1 = relationship("TournamentTeam", foreign_keys=[team1_id], back_populates="matches_as_team1")
    team2 = relationship("TournamentTeam", foreign_keys=[team2_id], back_populates="matches_as_team2")
    ...
    winner_advances_to = relationship("TournamentMatch", remote_side=[id], foreign_keys=[winner_advances_to_match_id])
    loser_advances_to = relationship("TournamentMatch", remote_side=[id], foreign_keys=[loser_advances_to_match_id])
...
class TournamentTrophy(Base):
    ...
    trophy_type = Column(String, nullable=False)  # "WINNER", "RUNNER_UP", "SEMI_FINALIST"
